...
export class ImageEncryptService {
  ...
  async encodeData(meta: ImageMetaInfo, files: File[], imageData: ImageData): Promise<{ meta: ImageMetaInfo, imageData: ImageData }> {
  ...
  async decodeData(imageData: ImageData): Promise<{ meta: ImageMetaInfo, files: File[] }> {
    ...
    const meta: ImageMetaInfo = JSON.parse(metadataText);
